...
try{
	...
	$retour["message"] = "Connexion à la base de données impossible";
			...
			$requete = $pdo->prepare('UPDATE user set lastname=:lastname WHERE email="'.$email.'" ');
			$requete->bindValue(':lastname', $_POST['lastname'], PDO::PARAM_STR);			

	if( $requete->execute() ){						//si la requete s'execute un code et un message sont envoyer
							$retour["success"] = true;
							$retour["message"] = "le nom a ete MAJ.";								
